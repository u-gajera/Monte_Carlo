function [Ms, Cs, Mx, Es, time1] = plusl(model,Lat,J1,J2,J3,D,A,Sp,L,estep,mstep,Ts,para,kb)
...
%set(bar,'CloseRequestFcn',['k = 3;','delete(bar);','return;']);
%barc = get(bar,'children');
%position = get(barc(2),'pos');
%k = 1;
%barc(3) = uicontrol(bar,'style','push',...
%    'pos',[170 10 60 22],...
%    'string','Pause','callback',...
%    ['if k==2',...
%        'k=1;',....
%        'set(barc(3),''string'',''Pause'');',...
%    'elseif k==1',...
%        'k=2;',...
%        'set(barc(3),''string'',''Continue'');',...
%    'end']);
%set(barc(2),'callback',['k = 3;','delete(bar);']);
...
for t = 1:size(Ts,2)   %change into parfor if parellel
    ...
    %    break
    %end
    %waitbar(t/size(Ts,2),bar,sprintf('%12.0f''K',Ts(t)))
    ...
    Ea = - 2 * Sp^2 * sum(gridTC(1,1,:)     .* ...
                         (J1 .* neighbors1   + ...
                          J2 .* neighbors2   + ...
                          J3 .* neighbors3)) + ...
         - D * Sp^2 * gridTC(1,1,3).^2       + ...
         - 2 * Sp^2 * sum(gridTC(1,1,3)     .* ...
                         (A  .* neighbors1(1,1,3)));
    Eb = - 2 * Sp^2 * sum(gridC(row,col,:)  .* ...
                         (J1 .* neighbors1   + ...
                          J2 .* neighbors2   + ...
                          J3 .* neighbors3)) + ...
         - D * Sp^2 * gridC(row,col,3).^2    + ...
         - 2 * Sp^2 * sum(gridC(row,col,3)  .* ...
                         (A  .* neighbors1(1,1,3)));
    dE = Ea - Eb ;
        ...
        if Lat == 'c'
            ...
            Eini = -2*Sp^2*sum(sum(sum(gridC           .* ...
                                      (J1 * shift1      + ...
                                       J2 * shift2      + ...
                                       J3 * shift3 )))) + ...
                 -Sp^2*D*sum(sum(gridC(:,:,3).^2))      + ...
                 -2*Sp^2*sum(sum(sum(gridC(:,:,3)      .* ...
                                    (A*shift1(:,:,3)))));
        elseif Lat == 't'
            shift1 = circshift(gridC, [ 0  1]) + ...
                     circshift(gridC, [ 1  0]) + ...
